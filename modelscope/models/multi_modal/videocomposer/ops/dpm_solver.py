        ...
        ***
        ...
        1. For discrete-time DPMs:
            ...
            **Important**:  Please pay special attention for the args for `alphas_cumprod`:
                ...
                In fact, we have alpha_{t_n} = \sqrt{\hat{alpha_n}},
                and log(alpha_{t_n}) = 0.5 * log(\hat{alpha_n}).
        ...
        if schedule == 'discrete':
            ...
            self.schedule = schedule
            if schedule == 'cosine':
                # For the cosine schedule, T = 1 will have numerical issues.
                # So we manually set the ending time T.
                # Note that T = 0.9946 may be not the optimal setting.
                # However, we find it works well.
                self.T = 0.9946
            else:
                self.T = 1.
...
def model_wrapper_guided_diffusion(model,
    ...
    DPM-Solver needs to solve the continuous-time diffusion ODEs. For DPMs trained on discrete-time labels,
    we need to firstly wrap the model function to a noise
    prediction model that accepts the continuous time as the input.
...
class DPM_Solver:
    ...
    def denoise_to_zero_fn(self, x, s):
        ...
        which is equivalent to solve the ODE from lambda_s to infty by first-order discretization.
        """
    ...
    def dpm_solver_first_update(self,
        ...
        Args:
            ...
            model_s: A pytorch tensor. The model function evaluated at time `s`.
                If `model_s` is None, we evaluate the model by `x` and `s`;
    ...
    def singlestep_dpm_solver_second_update(self,
        ...
        Args:
            ...
            return_intermediate: A `bool`. If true,
                                 also return the model value at time `s` and `s1` (the intermediate time).
    ...
    def singlestep_dpm_solver_third_update(self,
        ...
        if self.algorithm_type == 'dpmsolver++':
            ...
            if model_s1 is None:
                ...
                model_s1 = self.model_fn(x_s1, s1)
            x_s2 = ((sigma_s2 / sigma_s) * x -
                    (alpha_s2 * phi_12) * model_s + r2 / r1 *
    ...
    def dpm_solver_adaptive(self,
        ...
        Args:
            ...
            solver_type: either 'dpmsolver' or 'taylor'.
                         The type for the high-order solvers.
                        The type slightly impacts the performance. We recommend to use 'dpmsolver' type.
        Returns:
    ...
    def inverse(self,
        ...
        assert t_0 > 0 and t_T > 0, 'Time range needs to be greater than 0. \
            For discrete-time DPMs, it needs to be in [1 / N, 1], where N is the length of betas array'
