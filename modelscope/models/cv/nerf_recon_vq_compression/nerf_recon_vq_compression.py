...
from .renderer import OctreeRender_trilinear_fast, evaluation as evaluation_render, render_path
...
class NeRFReconVQCompression(TorchModel):
    ...
    def __init__(self, model_dir=None, **kwargs):
        ...
        self.test_dataset = dataset(self.data_dir,
                            split='test',
    ...
    def evaluation(self, render_dir, N_vis=-1):
        ...
        evaluation_test = partial(evaluation_render,test_dataset=self.test_dataset,
                            renderer=self.renderer,white_bg=white_bg, ndc_ray=ndc_ray,device=self.device,
                            compute_extra_metrics=True, im_save=True)
    ...
    def render_path(self, render_dir, N_vis=120):
        ...
        render_path(self.test_dataset, self.model, render_poses, self.renderer, 
                    savePath=f"{logfolder}/render_path", white_bg=white_bg, ndc_ray=ndc_ray, device=self.device)
