...
class TestGPT3BaseTrainDDP(DistributedTestCase):
    ...
    def setUp(self):
        ...
        self.model_dir = snapshot_download('damo/nlp_gpt3_text-generation_chinese-base')
        config:Config = read_config(os.path.join(self.model_dir, 'configuration.json'))
        config.megatron.world_size = 4
        config.megatron.tensor_model_parallel_size = 2
        # d = w // p // t
        config.megatron.data_parallel_size = 2
    ...
    def test_finetune_poetry(self):
        dist_start_cmd1,dist_start_cmd2 = dist_start_cmd_helper()
        self.start(
            finetune_poetry, num_gpus=4, dist_start_cmd=dist_start_cmd1)
        self.start(
            finetune_poetry, num_gpus=4, dist_start_cmd=dist_start_cmd2)

    @unittest.skipUnless(test_level() >= 0, 'skip test in current test level')
    def test_gpt3_base_evaluate_poetry(self):
        dist_start_cmd1,dist_start_cmd2 = dist_start_cmd_helper()
        self.start(
            evaluate_poetry, num_gpus=4, dist_start_cmd=dist_start_cmd1)
        self.start(
            evaluate_poetry, num_gpus=4, dist_start_cmd=dist_start_cmd2)
    ...
    def test_gpt3_base_predict_poetry(self):
        dist_start_cmd1,dist_start_cmd2 = dist_start_cmd_helper()
        self.start(
            predict_poetry, num_gpus=4, dist_start_cmd=dist_start_cmd1)
        self.start(
            predict_poetry, num_gpus=4, dist_start_cmd=dist_start_cmd2)

    @unittest.skipUnless(test_level() >= 0, 'skip test in current test level')
    def test_gpt3_base_output_pipeline(self):
        dist_start_cmd1,dist_start_cmd2 = dist_start_cmd_helper()
        self.start(
            pipeline_gpt3_base_output, num_gpus=4, dist_start_cmd=dist_start_cmd1)
        self.start(
            pipeline_gpt3_base_output, num_gpus=4, dist_start_cmd=dist_start_cmd2)
...
def pipeline_gpt3_base_output(work_dir='./gpt3_poetry'):
    ...
    with open(fr'{work_dir}/nlp_gpt3_text-generation_chinese-base_pipeline_gen_text.txt','w',encoding='utf-8') as f:
        f.write(gen_content)

def dist_start_cmd_helper():
    h_ip = socket.gethostbyname(socket.gethostname())
    dist_start_cmd1 = 'torchrun --nproc_per_node=2 --nnodes=2 --node_rank=0  --master_addr=\'%s\' --master_port=%s' % (h_ip, _find_free_port())
    dist_start_cmd2 = 'torchrun --nproc_per_node=2 --nnodes=2 --node_rank=1  --master_addr=\'%s\' --master_port=%s' % (h_ip, _find_free_port())
    return dist_start_cmd1,dist_start_cmd2
