...
from gaussian_renderer.utils import inverse_sigmoid, get_expon_lr_func, build_rotation
from gaussian_renderer.utils import mkdir_p
from gaussian_renderer.utils import RGB2SH
from gaussian_renderer.utils import BasicPointCloud
from gaussian_renderer.utils import strip_symmetric, build_scaling_rotation
...
class GaussianModel:
    ...
    def save_ply(self, path):
        ...
        f_dc = self._features_dc.detach().transpose(1, 2)
        f_dc = f_dc.flatten(start_dim=1).contiguous().cpu().numpy()
        f_rest = self._features_rest.detach().transpose(1, 2)
    ...
    def load_ply(self, path):
        ...
        self._features_dc = nn.Parameter(
            torch.tensor(
                features_dc,
                dtype=torch.float,
                device="cuda").transpose(1, 2).contiguous().requires_grad_(
                True))
        self._features_rest = nn.Parameter(
            torch.tensor(
                features_extra,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_xyz = torch.bmm(rots,
                            samples.unsqueeze(-1)).squeeze(-1) + \
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        # Extract points that satisfy the gradient condition
        selected_pts_mask = torch.where(torch.norm(grads, dim=-1) >= grad_threshold,
                                        True, False)
        ...
        self.densification_postfix(new_xyz, new_features_dc,
                                   new_features_rest,
                                   new_opacities, new_scaling,
                                   new_rotation)

    def densify_and_prune(self, max_grad, min_opacity,
                          extent, max_screen_size):
