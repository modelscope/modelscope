...
from gaussian_renderer.utils import safe_state, eval_sh
from gaussian_renderer.arguments import ModelParams, \
    PipelineParams, get_combined_args
from gaussian_renderer.scene.scene import Scene
...
def render_set(model_path, name, iteration, views, gaussians, pipeline,
    ...
    render_path = os.path.join(model_path, name,
                               "ours_{}".format(iteration), "renders")
    gts_path = os.path.join(model_path, name,
                            "ours_{}".format(iteration), "gt")
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        torchvision.utils.save_image(gt,
                                     os.path.join(gts_path,
                                                  '{0:05d}'.format(idx) + ".png"))


def render_datasets(dataset: ModelParams,
                    render_dir: str,
                    iteration: int,
    ...
    with torch.no_grad():
        ...
        if not skip_train:
            render_set(render_dir, "train",
                       scene.loaded_iter,
                       scene.getTrainCameras(), gaussians, pipeline,
                       background)

        if not skip_test:
            render_set(render_dir, "test",
                       scene.loaded_iter,
...
if __name__ == "__main__":
    ...
    render_datasets(model.extract(args),
                    args.iteration,
