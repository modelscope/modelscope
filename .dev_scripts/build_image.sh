...
docker_file_content="${docker_file_content} \nRUN export COMMIT_ID=$BUILD_HASH_ID && pip install --no-cache-dir -U adaseq pai-easycv &&  pip install --no-cache-dir -U git+https://github.com/modelscope/ms-swift.git@release/2.4 'decord' 'qwen_vl_utils' 'pyav' 'librosa' 'funasr' autoawq 'timm>0.9.5' 'accelerate' 'peft' 'optimum' 'trl' && pip install git+https://github.com/huggingface/transformers.git"
