...
from modelscope.outputs import (MachineReadingComprehensionOutput,
                                OutputKeys)
...
@MODELS.register_module(
    Tasks.machine_reading_comprehension, module_name=TaskModels.machine_reading_comprehension)
class ModelForMachineReadingComprehension(TorchModel):
    ...
    def __init__(self, model_dir: str, *args, **kwargs):
        ...
        self.span_transfer = MultiNonLinearProjection(self.config.hidden_size, self.config.hidden_size, self.config.hidden_dropout_prob,
                                                       intermediate_hidden_size=self.config.projection_intermediate_hidden_size)
        self.load_state_dict(torch.load(os.path.join(model_dir, ModelFile.TORCH_MODEL_BIN_FILE)))
