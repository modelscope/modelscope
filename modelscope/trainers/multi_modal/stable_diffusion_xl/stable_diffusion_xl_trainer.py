...
class SwiftDiffusionXLCheckpointProcessor(CheckpointProcessor):
...
@TRAINERS.register_module(module_name=Trainers.stable_diffusion_xl)
class StableDiffusionXLTrainer(EpochBasedTrainer):

    def __init__(self, *args, **kwargs):
        """Stable Diffusion XL trainers for fine-tuning.
        ...
        if use_swift:
            ckpt_hook = list(
                filter(lambda hook: isinstance(hook, CheckpointHook),
                       self.hooks))[0]
            ckpt_hook.set_processor(SwiftDiffusionXLCheckpointProcessor())
