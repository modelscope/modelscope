...
class TestStableDiffusionXLTrainer(unittest.TestCase):
    ...
    @unittest.skipUnless(test_level() >= 1,
                         'skip test because oom in github ci')
    def test_stable_diffusion_train(self):
        ...
        trainer = build_trainer(
            name=Trainers.stable_diffusion_xl, default_args=kwargs)
        trainer.train()
        result = trainer.evaluate()
        print(f'Stable-Diffusion-XL train output: {result}.')
    ...
    def test_stable_diffusion_eval(self):
        ...
        trainer = build_trainer(
            name=Trainers.stable_diffusion_xl, default_args=kwargs)
        result = trainer.evaluate()
        print(f'Stable-Diffusion-XL eval output: {result}.')
