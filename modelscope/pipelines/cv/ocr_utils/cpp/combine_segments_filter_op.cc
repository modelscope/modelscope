...
REGISTER_OP("CombineSegmentsFilter")
...
class CombineSegmentsFilterOp : public OpKernel {
  typedef std::array<T, 5> rbox_t;
  typedef std::array<T, 6> seg_t;
  typedef std::array<T, 2> point_t;

 public:
  explicit CombineSegmentsFilterOp(OpKernelConstruction* context)
    : OpKernel(context),
  ...
  void Compute(OpKernelContext* context) override {
    ...
    BatchCombineSegmentsFilter(segments.tensor<T, 3>(),
   ...
   */
  void BatchCombineSegmentsFilter(typename TTypes<T, 3>::ConstTensor segments,
    ...
    for (int i = 0; i < batch_size; ++i) {
      ...
      CombineSegmentsFilter(segments_i, group_indices_i, count, &combined_rboxes);
   ...
   */
  void CombineSegmentsFilter(const T* segments,
    ...
    for (const auto& kv : groups) {
      ...
      if (static_cast<T>(segments.size()) > 1) {
        // find mx + b
...
REGISTER_KERNEL_BUILDER(Name("CombineSegmentsFilter").Device(DEVICE_CPU),
                        CombineSegmentsFilterOp<CPUDevice, float>)
