...
class AdainResBlk(nn.Module):
    def __init__(self, dim_in, dim_out, style_dim=64, w_hpf=0,
                 actv=nn.LeakyReLU(0.2), upsample='none',out_for_onnx=False):
        super().__init__()
        self.w_hpf = w_hpf
        self.actv = actv
...
class Generator(nn.Module):
    def __init__(self, dim_in=48, style_dim=48, max_conv_dim=48 * 8,out_for_onnx=False):
        ...
        for lid in range(repeat_num):
            ...
            self.encode.append(
                ResBlk(dim_in, dim_out, normalize=True, downsample=_downtype,out_for_onnx=out_for_onnx))
            self.decode.insert(
                0, AdainResBlk(dim_out, dim_in, style_dim,
                               w_hpf=1, upsample=_downtype,out_for_onnx=out_for_onnx))  # stack-like
            dim_in = dim_out

        # bottleneck blocks (encoder)
        for _ in range(2):
            self.encode.append(
                ResBlk(dim_out, dim_out, normalize=True,out_for_onnx=out_for_onnx))

       
        # bottleneck blocks (decoder)
        for _ in range(2):
            self.decode.insert(
                0, AdainResBlk(dim_out , dim_out , style_dim, w_hpf=1,out_for_onnx=out_for_onnx))
...
class Generator2(nn.Module):
    def __init__(self, dim_in=48, style_dim=48, max_conv_dim=48 * 8,num_spk=1883, w_hpf=1, F0_channel=0,out_for_onnx=False):
        ...
        for lid in range(repeat_num):
            ...
            self.decode.insert(
                0, AdainResBlk(dim_out, dim_in, style_dim,
                               w_hpf=w_hpf, upsample=_downtype,norm=False))  # stack-like
            dim_in = dim_out
        ...
        for _ in range(2):
            self.decode.insert(
                0, AdainResBlk(dim_out + int(F0_channel / 2), dim_out + int(F0_channel / 2), style_dim, w_hpf=w_hpf,norm=False))
