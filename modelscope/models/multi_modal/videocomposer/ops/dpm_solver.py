...
def model_wrapper_guided_diffusion(
    ...
    We support three types of the diffusion model by setting `mean_type`:
        ...
        1. "x_{t-1}": previous data prediction model. (Trained by predicting the data x_{t-1} at time t-1).
    ...
    def noise_pred_fn(xt, t_continuous, **model_kwargs):
        ...
        if mean_type == 'eps':
            ...
            x0 = expand_dims(1. / posterior_mean_coef1, dims) * mu - expand_dims(posterior_mean_coef2 / posterior_mean_coef1, dims) * xt
            eps = (xt - expand_dims(alpha_t, dims) * x0) / expand_dims(sigma_t, dims)
        elif mean_type == 'x0':
            alpha_t, sigma_t = noise_schedule.marginal_alpha(t_continuous), noise_schedule.marginal_std(t_continuous)
            dims = out.dim()
            eps = (xt - expand_dims(alpha_t, dims) * out) / expand_dims(sigma_t, dims)
    ...
    def model_fn(x, t_continuous):
        ...
        if guide_scale is None:
            ...
            eps = u_out[:, :dim] + guide_scale * (y_out[:, :dim] - u_out[:, :dim])
        return eps
...
class DPM_Solver:
    ...
    def data_prediction_fn(self, x, t):
        ...
        if self.percentile is not None:
            s = torch.quantile(torch.abs(x0).reshape((x0.shape[0], -1)), self.percentile, dim=1)
