...
# Part of the implementation is borrowed and modified from MP-SENet, public available at https://github.com/yxlu-0102/MP-SENet
...
class SubPixelConvTranspose2d(nn.Module):
    def __init__(self,
                 in_channels, out_channels, kernel_size=(1, 3), stride=(1, 2), padding=(0, 1)):
        super(SubPixelConvTranspose2d, self).__init__()
        self.upscale_width_factor = stride[1]
        self.conv1 = nn.Conv2d(in_channels, out_channels * self.upscale_width_factor,
                               kernel_size=kernel_size, padding=padding)  # only change the width
...
class MappingDecoder(BaseDecoder):
    def __init__(self, h, out_channel=1):
        ...
        self.mask_conv = nn.Sequential(
            self.upsample_module_class(h.dense_channel, h.dense_channel, (1, 3), (1, 2)),
...
class PhaseDecoder(BaseDecoder):
    def __init__(self, h, out_channel=1):
        ...
        self.phase_conv_r = nn.Conv2d(h.dense_channel, out_channel, decoder_final_kersize)
        self.phase_conv_i = nn.Conv2d(h.dense_channel, out_channel, decoder_final_kersize)
