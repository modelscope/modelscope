...
class BatchSampler(Sampler):
    ...
    """
    def __init__(self, dataset_size, batch_size, num_replicas=None, rank=None, shuffle=False, seed=None):
        ...
        self.batches_per_rank = ceil_divide(dataset_size, self.num_replicas * self.batch_size)
...
class GroupSampler(Sampler):
    ...
    def __init__(self, group_file, batch_size, alpha=0.7, update_interval=5000, seed=8888):
    ...
    def __iter__(self):
        while True:
            ...
            # sample a batch
            batch = self.rng.choice(items, self.batch_size, replace=False if len(items) >= self.batch_size else True)
    ...
    def sample(self):
        scales = np.array([float(next(iter(u)).split(':')[-1]) for u in self.groups])
        p = scales ** self.alpha / (scales ** self.alpha).sum()
        group = self.rng.choice(self.groups, p=p)
        list_file = osp.join(self.group_folder, self.rng.choice(next(iter(group.values()))))
