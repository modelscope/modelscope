...
class PatchImageBuilder(Builder):
    """Refresh only modelscope/modelscope-hub on an already built image.

    A patch build inherits every dependency layer from ``--base_image``, which
    must be an image produced by a previous full build, and republishes it with
    the modelscope version segment of the tag replaced.
    """
    ...
    # prefixed(cuda12.8.1, torch2.10.0) or non numeric(test, latest).
    ...
    def init_args(self, args: Any) -> Any:
        ...
        if not base_image:
            raise ValueError(
                'A patch build must inherit an already built image. Pass '
                '--base_image <repository>:<tag> pointing at the image to '
                'patch.')
        ...
        if f'{docker_registry}:{self._output_tag}' == args.base_image:
            raise ValueError(
                'The patch build would overwrite its own base image '
    ...
    def _base_tag_of(cls, base_image: str) -> str:
        ...
        if not separator or not tag or '/' in tag:
            raise ValueError(
                '--base_image must carry the tag of the image to patch, e.g. '
                '<repository>:ubuntu22.04-py312-torch2.10.0-1.40.0-test, got: '
                f'{base_image}')
        return tag

    def _parse_base_tag(self, args: Any) -> None:
        tag = self._base_tag_of(args.base_image)
        # The build timestamp of the base image must not leak into the patch
        # tag; this build stamps its own.
        segments = [
    ...
    def _derive_tag(self, version: str, timestamped: bool = False) -> str:
        ...
        if timestamped:
            # Keep the historical layout where the timestamp precedes the
            # trailing '-test' marker.
            index = (
                len(segments) - 1 if segments[-1] == 'test' else len(segments))
            segments.insert(index, formatted_time)
    ...
    def generate_dockerfile(self) -> str:
        ...
        with open('docker/Dockerfile.patch', 'r') as f:
            content = f.read()
        content = content.replace('{base_image}', self.args.base_image)
    ...
    def build(self) -> int:
        print('=' * 60)
        print(f'Patch base image: {self.args.base_image}')
        print(f'Patch output image: {self.image()}')
        print('=' * 60)
    ...
    def push(self) -> int:
        ...
        if ret != 0:
            return ret
        print(f'Patch image timestamp tag: {image_tag2}')
        return self.run_cmd('docker', 'push', image_tag2)
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help='Patch build only: the modelscope version segment inside the '
    '--base_image tag. Auto detected when omitted.')
parser.add_argument(
    '--modelscope_hub_version',
    type=str,
    default=None,
    help='Patch build only: modelscope-hub requirement to install, e.g. 0.4.3 '
...
parser.add_argument(
    ...
    choices=['pypi', 'git'],
    default='pypi',
    help='Patch build only: install modelscope from the released PyPI version '
...
elif args.image_type.lower() == 'patch':
    builder_cls = [PatchImageBuilder]
else:
