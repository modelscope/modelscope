...
namespace lanms {
	...
	float poly_iou(const Polygon &a, const Polygon &b) {
        ...
        float a_area = 0,
              b_area = 0;
        a_area = cl::Area(a.poly);
        b_area = cl::Area(b.poly);
		return std::abs(inter_area) / std::max(std::abs(std::min(a_area, b_area)), 1.0f);
	}
		...
		merge_quadrangle_n9(const float *data, size_t n, float iou_threshold) {
			...
			for (size_t i = 0; i < n; i ++) {
				...
				if (polys.size()) {
					...
					if (0) { //(should_merge(poly, bpoly, iou_threshold)) {
						PolyMerger merger;
						merger.add(bpoly);
						merger.add(poly);
						bpoly = merger.get();
