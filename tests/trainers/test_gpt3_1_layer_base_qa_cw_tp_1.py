...
class TestGPT3OneLayerBaseQAandCWtp1(DistributedTestCase):
    ...
    def setUp(self):
        ...
        config.megatron.world_size = 1
        config.megatron.tensor_model_parallel_size = 1
    ...
    def test_1_layer_evaluate_poetry(self):
        dist_start_cmd = 'torchrun --nproc_per_node 1'
        self.start(
            evaluate_poetry_tp_1, num_gpus=1, dist_start_cmd=dist_start_cmd)
    ...
    def test_1_layer_output_pipeline_poetry(self):
        dist_start_cmd = 'torchrun --nproc_per_node 1'
        self.start(
            pipeline_poetry_tp_1, num_gpus=1, dist_start_cmd=dist_start_cmd)
    ...
    def test_1_layer_evaluate_dureader(self):
        dist_start_cmd = 'torchrun --nproc_per_node 1'
        self.start(
            evaluate_dureader_tp_1, num_gpus=1, dist_start_cmd=dist_start_cmd)
    ...
    def test_1_layer_output_pipeline_dureader(self):
        dist_start_cmd = 'torchrun --nproc_per_node 1'
        self.start(
            pipeline_dureader_tp_1, num_gpus=1, dist_start_cmd=dist_start_cmd)
...
gpt3_one_layer_cw_tp_1=GPT3OneLayerQAandCW(subtask='cw',tp=1,work_dir='./gpt3_poetry_tp_1')
def finetune_poetry_tp_1():
    ...
    gpt3_one_layer_cw_tp_1.finetune(max_epochs=max_epochs,cfg_modify_fn=cfg_modify_fn)
...
def pipeline_poetry_tp_1():
    pipe=gpt3_one_layer_cw_tp_1.pipeline()
    input='窗含西岭千秋雪'
    gen_content = pipe(input, max_length=128)
    with open(fr'{gpt3_one_layer_cw_tp_1.work_dir}/gpt3_1_layer_base_tp1_cw_pipeline_gen_text.txt', 'w', encoding='utf-8') as f:
        f.write(gen_content)

gpt3_one_layer_qa_tp_1=GPT3OneLayerQAandCW(subtask='qa',tp=1,work_dir='./dureader_tp_1')
...
def finetune_dureader_tp_1():
    ...
    gpt3_one_layer_qa_tp_1.finetune(max_epochs=max_epochs,cfg_modify_fn=cfg_modify_fn)
...
def pipeline_dureader_tp_1():
    pipe = gpt3_one_layer_qa_tp_1.pipeline()
    input1 = '推荐一下防脱发的洗发水'
    input2='重新推荐一次'
    gen_content1 = pipe(input1, max_length=128)
    with open(fr'{gpt3_one_layer_qa_tp_1.work_dir}/gpt3_1_layer_base_tp1_qa_pipeline_gen_text.txt', 'aw', encoding='utf-8') as f:
