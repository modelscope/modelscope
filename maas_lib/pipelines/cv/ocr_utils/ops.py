...
def conv2d(x, n_in, n_out, ksize, stride=1, padding='SAME',
  ...
  with tf.variable_scope(scope or 'conv2d'):
    ...
    kernel = _nn_variable('weight', [ksize,ksize,n_in,n_out], weight_init,
                          ...
                          **kwargs)
    yc = tf.nn.conv2d(input_padded, kernel, [1,stride,stride,1], padding='VALID')
    # add bias
    #print(^^^^^^^^^^^^^^^^^^^^^^^,bias)
    if bias == True:
...
def group_conv2d_relu(x, n_in, n_out, ksize, stride=1, group=4, padding='SAME',
        weight_init=None, bias=True, relu=False, name="group_conv2d",
        **kwargs):
    ...
    for i in range(group):
        conv_split, relu_split = conv2d(splits[i], n_in/group, n_out/group, ksize=ksize, stride=stride, padding=padding,
            weight_init=weight_init, bias=bias, relu=relu, scope='%s_%d'%(name,i))
        conv_list.append(conv_split)
    conv = tf.concat(values=conv_list, axis=group_axis, name=name+'_concat')
    relu =  tf.nn.relu(conv)
    return conv, relu

def group_conv2d_bn_relu(x, n_in, n_out, ksize, stride=1, group=4, padding='SAME',
        weight_init=None, bias=True, relu=False, name="group_conv2d",
        **kwargs):
    group_axis = len(x.get_shape())-1
    splits = tf.split(x, [int(n_in/group)]*group, group_axis)
...
def next_conv(x, n_in, n_out, ksize, stride=1, group=4, padding='SAME',
        weight_init=None, bias=True, relu=False, name="next_conv2d",
        **kwargs):
    conv_a, relu_a = conv_relu(x, n_in, n_in/2, ksize=1, stride=1, padding=padding,
        weight_init=weight_init, bias=bias, relu=relu, scope=name+'_a',
        **kwargs)

    conv_b, relu_b = group_conv2d_relu(relu_a, n_in/2, n_out/2, ksize=ksize, stride=stride, group=group, padding=padding,
        weight_init=weight_init, bias=bias, relu=relu, name=name+'_b',
        **kwargs)

    conv_c, relu_c = conv_relu(relu_b, n_out/2, n_out, ksize=1, stride=1, padding=padding,
        weight_init=weight_init, bias=bias, relu=relu, scope=name+'_c',
        **kwargs)

    return conv_c, relu_c

def next_conv_bn(x, n_in, n_out, ksize, stride=1, group=4, padding='SAME',
        weight_init=None, bias=True, relu=False, name="next_conv2d",
        **kwargs):
    conv_a, relu_a = conv_bn_relu(x, n_in, n_in/2, ksize=1, stride=1, padding=padding,
        weight_init=weight_init, bias=bias, relu=relu, scope=name+'_a',
        **kwargs)

    conv_b, relu_b = group_conv2d_bn_relu(relu_a, n_in/2, n_out/2, ksize=ksize, stride=stride, group=group, padding=padding,
        weight_init=weight_init, bias=bias, relu=relu, name=name+'_b',
        **kwargs)

    conv_c, relu_c = conv_bn_relu(relu_b, n_out/2, n_out, ksize=1, stride=1, padding=padding,
        weight_init=weight_init, bias=bias, relu=relu, scope=name+'_c',
        **kwargs)

    return conv_c, relu_c

def conv2d_ori(x, n_in, n_out, ksize, stride=1, padding='SAME',
           weight_init=None, bias=True, relu=False, scope=None,
           **kwargs):
  weight_init = weight_init or FLAGS.weight_init_method
  trainable = kwargs.get('trainable', True)
...
def get_combined_polygon(rboxes, resize_size):
  ...
  for i in range(rboxes.shape[0]):
    segment = np.reshape(np.array(utils.rboxes_to_polygons(rboxes)[i,:], np.int32), (-1,1,2))
...
'''
### combine_segments_curve in python version
def combine_segments_curve(segments_batch, group_indices_batch, segment_counts_batch, resize_size):
  combined_polygons_batch = [] # batch_size * num_combined_box(max) * polygon_dim(max)
  combined_polygons_dims_batch = [] # batch_size * num_combined_box(max)
  combined_counts = [] # batch_size

  for i in range(FLAGS.train_batch_size):
    combined_polygons = []
    combined_polygons_dims = []

    print('segment_counts_batch:' + str(segment_counts_batch[i]))
    if segment_counts_batch[i]>0:
      segments = segments_batch[i,:segment_counts_batch[i],:]
      group_indices = group_indices_batch[i,:segment_counts_batch[i]]
      combined_counts.append(np.max(group_indices)+1)
      print('combined_counts:' + str(combined_counts[i]))
      # input() #interval
      for j in range(combined_counts[i]):
        segments_group = segments[np.where(group_indices==j)[0],:]
        profile_polygon = get_profile_polygon(segments_group, resize_size)
        combined_polygons.append(profile_polygon)
        combined_polygons_dims.append(len(profile_polygon))
      polygon_dim = np.max(combined_polygons_dims)
      print('polygon_dim:' + str(polygon_dim))

      # padding
      for j in range(combined_counts[i]):
        if not combined_polygons_dims[j] == polygon_dim:
          combined_polygons[j] = np.hstack((combined_polygons[j], np.array((polygon_dim - combined_polygons_dims[j])*[0.0])))

      combined_polygons_batch.append(np.array(combined_polygons))
      combined_polygons_dims_batch.append(np.array(combined_polygons_dims))
    else:
      combined_counts.append(1)
      combined_polygons_batch.append(np.array([8*[0.0]]))
      combined_polygons_dims_batch.append(np.array([8]))

  num_combined_polygons_max = np.max(combined_counts)

  for i in range(FLAGS.train_batch_size):
    if not combined_counts[i] == num_combined_polygons_max:
      combined_polygons_dims_batch[i] = np.hstack((combined_polygons_dims_batch[i], np.array((num_combined_polygons_max - combined_counts[i])*[0])))

  for i in range(FLAGS.train_batch_size):
    if not np.max(combined_polygons_dims_batch[i]) == np.max(np.array(combined_polygons_dims_batch)):
      combined_polygons_batch[i] = np.hstack((combined_polygons_batch[i],
        np.array(combined_counts[i]*[(np.max(np.array(combined_polygons_dims_batch)) - np.max(combined_polygons_dims_batch[i]))*[0.0]])))
    if not combined_counts[i] == num_combined_polygons_max:
      combined_polygons_batch[i] = np.vstack((combined_polygons_batch[i],
        np.array((num_combined_polygons_max - combined_counts[i])*[np.max(np.array(combined_polygons_dims_batch))*[0.0]])))
  return np.asarray(combined_polygons_batch, np.float32), np.asarray(combined_counts, np.int32), np.asarray(combined_polygons_dims_batch, np.int32)

def get_profile_polygon(segments_group, resize_size):
  # single filter
  if segments_group.shape[0]>1:
    image_w = resize_size[1]
    image_h = resize_size[0]
    img = np.zeros((image_h, image_w, 3), np.uint8)
    for i in range(segments_group.shape[0]):
      segment = np.reshape(np.array(utils.rboxes_to_polygons(segments_group)[i,:], np.int32), (-1,1,2))
      cv2.drawContours(img, [segment], 0, (255,255,255), -1)
    #cv2.imwrite('/home/xixing.tj/visu/visu_mask/full_mask.jpg', img)
    img2gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    ret,thresh = cv2.threshold(img2gray,127,255,cv2.THRESH_BINARY)
    ...
    if len(contours) >0:
      ...
      for cont in contours:
        if cv2.contourArea(cont) > max_area:
          cnt = cont
          max_area = cv2.contourArea(cont)
      epsilon = 0.01*cv2.arcLength(cnt,True) # error rate 10%
  ...
  return np.asarray(np.array(approx).reshape([-1]), np.float32)
'''
...
def get_coord_link(offsets, map_size, offsets_defaults):
  if offsets < offsets_defaults[1][1]:
    ...
    offsets_node = (offsets - offsets_defaults[1][1]) // (N_LOCAL_LINKS + N_CROSS_LINKS) + offsets_defaults[1][0]
    link_idx = (offsets - offsets_defaults[1][1]) % (N_LOCAL_LINKS + N_CROSS_LINKS)
...
def get_neighbours(l_idx, x, y, map_size, offsets_defaults):
  if l_idx == 0:
    coord = [(0, x - 1, y - 1), (0, x, y - 1), (0, x + 1, y - 1),
      (0, x - 1, y),                 (0, x + 1, y),
      (0, x - 1, y + 1), (0, x, y + 1), (0, x + 1, y + 1)]
  else:
    coord = [(l_idx, x - 1, y - 1), (l_idx, x, y - 1), (l_idx, x + 1, y - 1),
      (l_idx, x - 1, y),                 (l_idx, x + 1, y),
      (l_idx, x - 1, y + 1), (l_idx, x, y + 1), (l_idx, x + 1, y + 1),
      (l_idx - 1, 2*x, 2*y), (l_idx - 1, 2*x + 1, 2*y), (l_idx - 1, 2*x, 2*y + 1), (l_idx - 1, 2*x + 1, 2*y + 1)]
  neighbours_offsets = []
  link_idx = 0
  for nl_idx, nx, ny in coord:
    if is_valid_coord(nl_idx, nx, ny, map_size):
      neighbours_offset_node = offsets_defaults[nl_idx][0] + map_size[nl_idx][1]*ny + nx
      if l_idx == 0:
        neighbours_offset_link = offsets_defaults[l_idx][1] + (map_size[l_idx][1]*y + x) * N_LOCAL_LINKS + link_idx
      else:
        neighbours_offset_link = offsets_defaults[l_idx][1] + (map_size[l_idx][1]*y + x) * (N_LOCAL_LINKS + N_CROSS_LINKS) + link_idx
      neighbours_offsets.append([neighbours_offset_node, neighbours_offset_link, link_idx])
    link_idx += 1
  # [node_offsets, link_offsets, link_idx(0-7/11)]
  return neighbours_offsets

def decode_segments_links_(image_size, all_nodes, all_links, all_reg, anchor_sizes):
  batch_size = FLAGS.test_batch_size
  # offsets = 12285 #768
  all_nodes_flat = tf.concat([tf.reshape(o, [batch_size, -1, N_SEG_CLASSES]) for o in all_nodes], axis=1)
  all_links_flat = tf.concat([tf.reshape(o, [batch_size, -1, N_LNK_CLASSES]) for o in all_links], axis=1)
  all_reg_flat = tf.concat([tf.reshape(o, [batch_size, -1, OFFSET_DIM]) for o in all_reg], axis=1)
  segments, group_indices, segment_counts, group_indices_all = tf.py_func(decode_batch,
    [all_nodes_flat, all_links_flat, all_reg_flat, image_size, tf.constant(anchor_sizes)], [tf.float32, tf.int32, tf.int32, tf.int32])
...
def decode_segments_links_train(image_size, all_nodes, all_links, all_reg, anchor_sizes):
...
def decode_batch(all_nodes, all_links, all_reg, image_size, anchor_sizes):
  ...
  for image_id in range(batch_size):
    if not batch_segments_counts[image_id] == max_count:
      batch_segments[image_id] = np.vstack((batch_segments[image_id], np.array((max_count - batch_segments_counts[image_id])*[(OFFSET_DIM)*[0.0]])))
      batch_group_indices[image_id] = np.hstack((batch_group_indices[image_id], np.array((max_count - batch_segments_counts[image_id])*[-1])))
  return np.asarray(batch_segments, np.float32), np.asarray(batch_group_indices, np.int32), np.asarray(batch_segments_counts, np.int32), np.asarray(batch_group_indices_all, np.int32)
...
def decode_image(image_node_scores, image_link_scores, image_reg, image_size, anchor_sizes):
  ...
  image_group_indices_all = decode_image_by_mutex(
    image_node_scores, image_link_scores, FLAGS.node_threshold, FLAGS.link_threshold, map_size, offsets_defaults)
...
def decode_image_by_join(node_scores, link_scores_raw, node_threshold, link_threshold, map_size, offsets_defaults):
...
def get_link_mask(node_mask, offsets_defaults, link_max):
  ...
  link_mask[0:offsets_defaults[1][1]] = np.tile(node_mask[0:offsets_defaults[1][0]],
    (N_LOCAL_LINKS, 1)).transpose().reshape(offsets_defaults[1][1])
  link_mask[offsets_defaults[1][1]:offsets_defaults[2][1]] = np.tile(node_mask[offsets_defaults[1][0]:offsets_defaults[2][0]],
    (N_LOCAL_LINKS + N_CROSS_LINKS, 1)).transpose().reshape((offsets_defaults[2][1] - offsets_defaults[1][1]))
  link_mask[offsets_defaults[2][1]:offsets_defaults[3][1]] = np.tile(node_mask[offsets_defaults[2][0]:offsets_defaults[3][0]],
    (N_LOCAL_LINKS + N_CROSS_LINKS, 1)).transpose().reshape((offsets_defaults[3][1] - offsets_defaults[2][1]))
  link_mask[offsets_defaults[3][1]:offsets_defaults[4][1]] = np.tile(node_mask[offsets_defaults[3][0]:offsets_defaults[4][0]],
    (N_LOCAL_LINKS + N_CROSS_LINKS, 1)).transpose().reshape((offsets_defaults[4][1] - offsets_defaults[3][1]))
  link_mask[offsets_defaults[4][1]:offsets_defaults[5][1]] = np.tile(node_mask[offsets_defaults[4][0]:offsets_defaults[5][0]],
    (N_LOCAL_LINKS + N_CROSS_LINKS, 1)).transpose().reshape((offsets_defaults[5][1] - offsets_defaults[4][1]))
  link_mask[offsets_defaults[5][1]:] = np.tile(node_mask[offsets_defaults[5][0]:],
    (N_LOCAL_LINKS + N_CROSS_LINKS, 1)).transpose().reshape((len(link_mask) - offsets_defaults[5][1]))
...
def get_link8(link_scores_raw, map_size):
  ...
  for i in range(N_DET_LAYERS):
    if i == 0:
      ...
      offsets_end = map_size[i][0]*map_size[i][1]*(N_LOCAL_LINKS+16)
      offsets_link = map_size[i][0]*map_size[i][1]*(N_LOCAL_LINKS+16)
      link8_mask[:offsets_start] = 1
    else:
      offsets_start = offsets_link + map_size[i][0]*map_size[i][1]*N_LOCAL_LINKS
      offsets_end = offsets_link + map_size[i][0]*map_size[i][1]*(N_LOCAL_LINKS+16)
      offsets_link_pre = offsets_link
      offsets_link += map_size[i][0]*map_size[i][1]*(N_LOCAL_LINKS+16+N_CROSS_LINKS)
...
def decode_image_by_mutex(node_scores, link_scores, node_threshold, link_threshold, map_size, offsets_defaults):
  node_mask = node_scores[:, POS_LABEL] >= node_threshold
  #link_scores = get_link8(link_scores_raw, map_size)
  ...
  offsets_link_max = np.argsort(-(link_max*link_mask*link_mask_th))[:len(offsets_pos_list) * 8]
  ...
  def find_mutex_constraint(point):
    ...
    if not mutex_point_list == []:
      for mutex_point in mutex_point_list:
        ...
        if not mutex_point in mutex_point_list_new:
  ...
  def combine_mutex_constraint(point, parent):
    ...
    for mutex_point in mutex_point_list:
      if not is_root(mutex_point):
        mutex_point = find_root(mutex_point)
      if not mutex_point in mutex_parent_list:
  ...
  def add_mutex_constraint(p1, p2):
    ...
    if not p1 in mutex_point_list2:
      mutex_point_list2.append(p1)
    if not p2 in mutex_point_list1:
  ...
  def join(p1, p2):
    ...
    if root1 != root2 and (not root1 in find_mutex_constraint(root2)):
      set_parent(root1, root2)
  ...
  for _, offsets_link in enumerate(offsets_link_max):
    l_idx, x, y, link_idx = get_coord_link(offsets_link, map_size, offsets_defaults)
    offsets = offsets_defaults[l_idx][0] + map_size[l_idx][1]*y + x
    if offsets in offsets_pos_list:
      neighbours = get_neighbours(l_idx, x, y, map_size, offsets_defaults)
      if not len(np.where(np.array(neighbours)[:,2]==link_idx)[0]) == 0:
        noffsets = neighbours[np.where(np.array(neighbours)[:,2]==link_idx)[0][0]]
