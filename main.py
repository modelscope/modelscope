...
from lora_diffusion.xformers_utils import \
...
def main(args):
    ...
    dataset, collate_fn, loss_calculator = DreamBoothTuner().tune(
        unet,
    ...
    unet_lora_params = LoRATuner().tune(
        model=unet, r=args.lora_rank, loras=args.resume_unet)
    if args.train_text_encoder:
        text_encoder_lora_params = LoRATuner().tune(
            model=text_encoder,
    ...
    text_lr = (
        args.learning_rate
        if args.learning_rate_text is None else args.learning_rate_text)
    ...
    ] if args.train_text_encoder else itertools.chain(*unet_lora_params))
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            loss = loss_calculator(model_pred, target)
                ...
                if args.save_steps and global_step - last_save >= args.save_steps:
                    if accelerator.is_main_process:
                        ...
                        accepts_keep_fp32_wrapper = 'keep_fp32_wrapper' in set(
                            inspect.signature(
                                accelerator.unwrap_model).parameters.keys())
                        extra_args = ({
                            'keep_fp32_wrapper': True
                        } if accepts_keep_fp32_wrapper else {})
                        ...
                        if args.train_text_encoder:
                            torch.save(pipeline.text_encoder.state_dict(),
